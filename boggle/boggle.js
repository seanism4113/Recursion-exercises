...
function makeBoard(boardString) {
    ...
    */
	...
	const board = [letters.slice(0, 5), letters.slice(5, 10), letters.slice(10, 15), letters.slice(15, 20), letters.slice(20, 25)];

	return board;
}

function find(board, word, x = null, y = null, wordIndex = 0, visited = new Set()) {
	if (x === null && y === null) {
		for (let i = 0; i < 5; i++) {
			for (let j = 0; j < 5; j++) {
	...
	if (x < 0 || x >= 5 || y < 0 || y >= 5 || board[x][y] !== word[wordIndex] || visited.has(`${x},${y}`)) {
		return false;
	}
	...
	for (const [dx, dy] of directions) {
		if (find(board, word, x + dx, y + dy, wordIndex + 1, visited)) {
			return true;
		}
	}
